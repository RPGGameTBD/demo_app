...
- (void) addButtons {
    ...
    //figure out screen dimensions for dynamic button placement
    ...
    for (int i = 0; i < [grantObjects count]; i++) {
        ...
        graphButton.frame = CGRectMake(screenHeight-120, i*25+20, buttonWidth, buttonHeight);
        [self.view addSubview:graphButton];
        [self.view bringSubviewToFront:graphButton];
        [graphButton setTitle:[[[grantObjects objectAtIndex:i ] getMetadata] objectForKey:@"title"] forState:UIControlStateNormal];
        ...
        graphButton.backgroundColor = [UIColor colorWithRed:components[0] green:components[1] blue:components[2] alpha:.4];
        [[graphButton layer] setCornerRadius:8.0f];
        [[graphButton layer] setMasksToBounds:YES];
        [[graphButton layer] setBorderWidth:1.0f];
        [graphButton.titleLabel setFont:[UIFont systemFontOfSize:8]];
        [graphButton addTarget:self action:@selector(buttonPress:) forControlEvents:UIControlEventTouchUpInside];
        [buttonReferences addObject:graphButton];
    }
}
